...
override release_files=README.md LICENSE Makefile $(plugins) $(sourcefiles) \
	$(includefiles) $(configs) $(gamedata) $(extensions) $(translations)
...
$(plugins_dir)/%.asm: %.sp | $(plugins_dir)
	$(SPCOMP) $^ -o$@ -a $(IFLAGS) $(SPFLAGS)

$(plugins_dir)%.lst: %.sp | $(plugins_dir)
	$(SPCOMP) $^ -o$@ -l $(IFLAGS) $(SPFLAGS)

$(plugins_dir):
	mkdir -p $@

release.tar.gz: $(release_files)
	tar cvzf $@ $^

release.zip: $(release_files)
	$(ZIP) $@ $^
...
.PHONY: all clean compile list install uninstall tags dos2unix
